...
resource "kubernetes_deployment" "pods_qa" {
  metadata {
...
resource "kubernetes_stateful_set" "db_qa" {
metadata {
    name = "prometheus"
    namespace = kubernetes_namespace.terra_ns.metadata[0].name
  }
